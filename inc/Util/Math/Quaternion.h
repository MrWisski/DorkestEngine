...
public:
	...
	{
		const Quaternion<T>& lhs = *this;
		return (fabsf(lhs.w - rhs.w) < EPSILON) && lhs.v == rhs.v;
	}
	...
	static Quaternion<T> fromAxisRot(Vector3<T> axis, float angleDeg)
	{
		...
		double sa2 = sinf(angleRad / 2);
		double ca2 = cosf(angleRad / 2);
	...
	Quaternion<T> slerp(T r, const Quaternion<T>& q2) const
	{
		...
		else
		{
			...
			{
				T rA = (T)sinf((1.0 - r) * theta) / sinTheta;
				T rB = (T)sinf(r * theta) / sinTheta;
